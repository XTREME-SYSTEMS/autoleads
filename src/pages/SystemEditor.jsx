...
import {
  Bot, Send, Loader2, Sparkles, Zap, ShieldCheck, Code, Wrench,
  Plus, MessageSquare, Trash2, ChevronRight, FileEdit,
} from "lucide-react";
import { base44 } from "@/api/base44Client";
import ReactMarkdown from "react-markdown";

const AGENT_NAME = "system_editor";

const QUICK_COMMANDS = [
  { label: "Improve This Proposal", text: "Read the most recent proposal and its linked project. Regenerate the line items with detailed descriptions that include square footage, specific scope names, and quantities. Add a scope of work section. Make the proposal professional and complete with proper construction proposal language.", icon: FileEdit },
  { label: "Enrich This Project", text: "Read the most recent project. Invoke verifyProject to fetch the source URL and extract plans, specs, drawings, documents, square footage, and floor finish. Update the project with all extracted data. Then invoke translateSourceDocument to generate a readable source document.", icon: Zap },
  { label: "Run Full Pipeline", text: "Run the full autonomous pipeline: scrape → verify → takeoff → estimate → proposal. Process all qualified projects through the complete pipeline.", icon: ChevronRight },
  { label: "Fix All Issues", text: "Fix every open SystemGap and SystemFlag. Auto-heal the system, fix broken projects and proposals, and verify all fixes.", icon: Wrench },
  { label: "Run System Audit", text: "Run a complete forensic audit of the entire AUTOLEADS system. Score every layer, identify every gap, and create SystemFlag records for all issues found.", icon: ShieldCheck },
  { label: "Regenerate All Proposals", text: "Read all proposals in internal_review status. For each one, read its linked project and takeoffs, then regenerate the line items with detailed descriptions including square footage, scope names, and quantities. Make each proposal professional with proper construction language.", icon: Code },
];
      ...
      <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${isUser ? "bg-[#f2df0d] text-black" : "bg-white border border-black/10 text-black"}`}>
        {!isUser && (
          <div className="mb-1 flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-black uppercase tracking-wide text-black/50">System Editor</span>
          </div>
        )}
        {message.content && (
          isUser
  ...
  const newConversation = async () => {
    try {
      const conv = await base44.agents.createConversation({
        ...
        metadata: { name: `Edit Session ${new Date().toLocaleString()}`, description: "System editor session" },
      });
  ...
  const send = async (text) => {
    ...
    try {
      ...
      if (!conv) {
        ...
        setConversations(prev => { const next = [conv, ...prev]; return next; });
        setActiveId(conv.id);
      }
      ...
      <div className="hidden w-72 shrink-0 flex-col border-r border-black/10 bg-white md:flex">
        <div className="border-b border-black/10 p-4">
          <div className="flex items-center gap-2.5">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-[#0b0b0b] text-[#f2df0d]"><Code size={22} /></span>
            <div>
              <h1 className="text-sm font-black">System Editor</h1>
              <p className="text-[10px] text-black/50">Full-access interactive AI</p>
            </div>
          </div>
        </div>
        <div className="p-3">
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-3">
          {conversations.length === 0 && <p className="px-2 py-4 text-center text-xs text-black/30">No sessions yet</p>}
          {conversations.map(c => (
        ...
        <div className="border-t border-black/10 p-3">
          <button onClick={() => navigate("/autonomous-executor")} className="flex w-full items-center gap-2 rounded-lg border border-black/15 px-3 py-2 text-xs font-bold hover:bg-black/5">
            <Sparkles size={13} /> Autonomous Executor
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col">
        ...
        <div className="flex items-center justify-between border-b border-black/10 bg-white px-4 py-3 md:hidden">
          <div className="flex items-center gap-2">
            <span className="grid h-8 w-8 place-items-center rounded-lg bg-[#0b0b0b] text-[#f2df0d]"><Code size={16} /></span>
            <span className="text-sm font-black">System Editor</span>
          </div>
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-6">
          {loading ? (
            ...
            <div className="flex h-full flex-col items-center justify-center text-center">
              <span className="grid h-16 w-16 place-items-center rounded-2xl bg-[#0b0b0b] text-[#f2df0d]"><Code size={32} /></span>
              <p className="mt-4 text-lg font-black">System Editor Ready</p>
              <p className="mt-1 max-w-md text-sm text-black/50">
                This AI has full system access. It can edit proposals, enrich projects, run the pipeline, fix issues, and manage every part of AUTOLEADS. Send a command or use a quick command above to begin.
              </p>
            </div>
          ) : (
              ...
              {sending && (
                <div className="flex justify-start">
                  <div className="flex items-center gap-2 rounded-2xl border border-black/10 bg-white px-4 py-3">
                    ...
                    <span className="text-xs text-black/40">Editor is working…</span>
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="border-t border-black/10 bg-white px-4 py-3">
          <div className="mx-auto flex max-w-3xl items-end gap-2">
            <textarea
              ...
              placeholder="Command the system editor… (e.g., 'Improve the OMAKAI proposal with detailed line items and square footage')"
          ...
          <p className="mx-auto mt-1.5 max-w-3xl text-center text-[10px] text-black/30">
            ⚠ Full-access agent — can edit, delete, send, and execute any system operation
          </p>
        </div>
      </div>
    </div>
