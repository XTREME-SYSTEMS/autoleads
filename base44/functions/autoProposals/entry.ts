...
async function processOrg(client: any, orgId: string, limit: number, singleProjectId: string | null) {
  ...
  // The user's default proposal template — generated by the AI Proposal Generator.
  // Its structure guides the LLM to produce proposals that match the company's branded standard.
  const templateAsset = (brandAssets || []).find((b: any) => b.type === 'proposal_template' && b.is_default) || null;
  const proposalTemplate = templateAsset?.content || '';
...
${proposalTemplate ? `STANDARD PROPOSAL TEMPLATE: Your company has a master proposal template. Follow its structure, tone, and section organization when generating the scope_of_work and cover_letter. The template includes sections like: Executive Summary, Company Overview, Scope of Work, Pricing Summary, Timeline, Qualifications, and Terms. Match this professional standard.

Template structure reference (first 2000 chars):
${proposalTemplate.slice(0, 2000)}` : ''}`,
